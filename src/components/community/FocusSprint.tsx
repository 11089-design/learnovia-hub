  ...
  return (
    <div className="rounded-2xl border border-border/60 bg-card p-3">
      <div className="flex items-center justify-between">
        <p className="text-[10px] font-semibold uppercase text-muted-foreground">Focus Sprint</p>
        <span className={`rounded-full px-2 py-0.5 text-[10px] font-semibold ${onBreak ? "bg-secondary text-secondary-foreground" : "bg-primary/10 text-primary"}`}>
          {onBreak ? "Break" : "Deep work"}
        </span>
      </div>
      <div className="mt-2 flex items-center gap-2">
        {onBreak ? <Coffee className="h-5 w-5 text-primary" /> : <Timer className="h-5 w-5 text-primary" />}
        <span className="font-display text-2xl font-bold tabular-nums">{mm}:{ss}</span>
      </div>
